  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    if use_batchnorm:
      for i in range(self.num_layers - 1):
        self.params['gamma' + str(i + 1)] = np.random.rand(hidden_dims[i])
        self.params['beta' + str(i + 1)] = np.random.rand(hidden_dims[i])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers + 1):
      if i != self.num_layers and self.use_batchnorm:
        prev_out, affine_batchnorm_relu_cache = affine_batchnorm_relu_forward(prev_out, self.params['W' + str(i)], self.params['b' + str(i)],\
          self.params['gamma' + str(i)], self.params['beta' + str(i)], self.bn_params[i - 1])
      else:
        prev_out, affine_relu_cache = affine_relu_forward(prev_out, self.params['W' + str(i)], self.params['b' + str(i)])
      dropout_cache = None
      if self.use_dropout:
        prev_out, dropout_cache = dropout_forward(prev_out, self.dropout_param)

      if i != self.num_layers and self.use_batchnorm:
        result.append((affine_batchnorm_relu_cache, dropout_cache))
      else:
        result.append((affine_relu_cache, dropout_cache))
    scores = prev_out
    ...
    for i in range(self.num_layers, 0, -1):
      ...
      if self.use_dropout:
        prev_dout = dropout_backward(prev_dout, result[i - 1][1])
      if i != self.num_layers and self.use_batchnorm:
        prev_dout, grads['W' + str(i)], grads['b' + str(i)], grads['gamma' + str(i)], grads['beta' + str(i)] =\
          affine_batchnorm_relu_backward(prev_dout, result[i - 1][0])
      else:  
        prev_dout, grads['W' + str(i)], grads['b' + str(i)] = affine_relu_backward(prev_dout, result[i - 1][0])
